...
function renderAllTasks(){
    for (const status of statuses) {
        ...
        if (tasks.length < 1){
            render(new CardStubComponent(),listComponent.getElement());//заглушка если нет задач
        }
        ...
        if (status === Status.BASKET) {
            render(new ClearBtn(taskService, disabledBtn), listComponent.getElement());//кнопка корзины в конце корзины
        }
    }
}
renderAllTasks();
